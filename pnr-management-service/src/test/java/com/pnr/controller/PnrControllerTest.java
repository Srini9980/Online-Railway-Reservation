...
public class PnrControllerTest {
	...
	public void testSavePnr() throws Exception {
		...
		mockMvc.perform(post("/pnr/save").contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(pnr))).andExpect(status().isOk());
	...
	public void testGetPnrByBookingId() throws Exception {
		...
		mockMvc.perform(get("/pnr/byid/10").contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(pnrResponse))).andDo(print());
